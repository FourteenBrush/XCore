...
public class Config implements Reloadable {
    ...
    public Config(XCore core, String fileName, boolean copyDefaults) {
        this.core = core;
        ...
        // won't save the file yet so this is safe to call
        reload();
    }
    ...
    public void reload() {
        ...
        InputStream defaults = core.getResource(fileName);
        if (defaults == null) {
            core.getLogger().warning("Tried to copy defaults for " + fileName + " but it doesn't exist");
            return;
        }

        Reader reader = new InputStreamReader(defaults);
    ...
    public void saveBlocking() {
        try {
            ...
            core.getLogger().log(Level.SEVERE, "Couldn't write to " + fileName, e);
        }
    }
